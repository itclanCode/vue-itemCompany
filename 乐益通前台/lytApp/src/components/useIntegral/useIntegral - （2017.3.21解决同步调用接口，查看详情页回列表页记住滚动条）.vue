  ...
  <div class="all_box">
    ...
    <div id="wrapper" v-show="useIntegral">
      <div id="scroller">
        ...
        <ul class="main_icon">
          ...
          <li>
            <router-link to="/mileage">
              <img src="./duilicheng_icon@2x.png" alt=""/>
        ...
        <!--<div>{{data}}</div>-->
        <ul class="classification retainbb">
          <!--<li v-for="(getAllType,$index) in getAllTypes" @click="toggle($index)" :class="{active:active==$index}">{{getAllType.name}}</li>-->
          <li>全部商品</li>
        </ul>
        <ul class="fictitious" id="Content">
          <li class="retainbb" v-for="getListByType in getListByTypes" @tap="jump()">
            <router-link :to="{path:'goods',query:{goodId:getListByType.id}}">
              ...
              <img :src="'/loyaltyApp'+getListByType.masterImg"/>
      ...
      toggle: function($index) {
        ...
        if(this.active !== $index){
          ...
          this.$http.post('/loyaltyApp/product/getListByType',{
            ...
            productTypeId: this.getAllTypes[$index].id
...
//          that.$http.get('/loyaltyApp/product/getListByType',{
//            pageNum:3
//          },{emulateJSON:true}).then((response) => {
...
//          });
          ...
          that.$http.get('/loyaltyApp/product/getListByType?pageNum='+that.page).then((response) => {
            response = response.body; // 通过response.josn来格式化json数据,这个返回的是promise对象
            if (response.resultCode == 200) {
...
//          if(response.resultCode == 200){
//            that.getListByTypes = that.getListByTypes.concat(response.resultBody);
//            console.log(that.getListByTypes);
//            //1s显示数据
//            setTimeout(function() {
...
//              that.getListByTypes = that.getListByTypes.concat(response.resultBody);
//            }else{
//              that.getListByTypes = response.resultBody
//            }
//            //1s显示数据
//            setTimeout(function() {
//              that.assignment(response.resultBody);
...
//      this.$http.get('/loyaltyApp/product/getListByType?pageNum='+this.num).then((response) => {
//        response = response.body; // 通过response.josn来格式化json数据,这个返回的是promise对象
//        console.log(response);
//        if(response.resultCode == 200){
//          that.getListByTypes = response.resultBody
...
//      });
      ...
      for(var i=0;i<that.page;i++){
        $.ajax({
          type:'GET',
          url:'/loyaltyApp/product/getListByType',
          data:{
            pageNum:i+1
          },
          dataType:'josn',
          ...
          success:function(data) {
            ...
            if(i<that.page){
              that.getListByTypes = that.getListByTypes.concat(data.resultBody);
            }
            if((i+1)== that.page){
              setTimeout(function () {
                that.assignment();
